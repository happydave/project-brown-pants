...
- **sounding** `/crates/app/` — the windowed application (binary `sounding`).
  - `/crates/app/src/main.rs` — a **scene dispatcher** (WI 514): selects a toy scene at launch (`EditorPlugin` by default, `PlanetPlugin` with the `planet` arg) and registers the common Toy 1–3 sim/bus plugins, which run headless behind whichever scene is shown. Inputs: keyboard + launch arg. Outputs: rendered window.
