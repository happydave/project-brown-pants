...
- **sounding** `/crates/app/` — the windowed application (binary `sounding`).
  - `/crates/app/src/main.rs` — the **Toy 5** voxel-editor scene (WI 505): an orbit camera + light hosting the `EditorPlugin`. The sim/bus plugins (Toys 1–3) stay registered and run headless behind it. Inputs: keyboard. Outputs: rendered window. (Earlier toys' scenes — Toy 1 2D, Toy 4 planet — are retired as the default but their modules are retained.)
  - `/crates/app/src/editor.rs` — the **Toy 5** voxel editor (WI 505): `EditorState` (craft + cursor + material + subassembly buffer) and `EditorPlugin`. Cursor-based add/remove of voxels and devices, material palette, blueprint/subassembly save·load·insert via the WI 498 format, and immediate-mode gizmo visualization (voxel cubes by material, cursor, CoM marker, principal-inertia axes, area-curve plot) redrawn each frame from the `VoxelCraft`. Consumes `sounding_sim::voxel`.
  - `/crates/app/src/floating_origin.rs` — floating-origin rebasing (WI 504), retained (`#![allow(dead_code)]`) for when in-world flight returns; its unit tests still run. Pure `render_translation(world_f64, anchor) -> Vec3` plus `WorldPlacement`/`FloatingOrigin`/`AnchorCamera`/`FloatingOriginPlugin`.
