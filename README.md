...
- `cargo run -p sounding -- workshop` — grounded build-and-test sandbox (WI 599/602/603): **Build** (voxel editor) ↔ **Test** (fly by hand on the textured ground with live collision — land, rest, drive, crash → shatter), toggle with `Enter`; in Test, `Backspace` rebuilds
