...
- `cargo run -p sounding -- dive` — the Toy 9 dive: a craft falls vacuum→atmosphere→ocean, drag/buoyancy/pressure all from one fluid field, now with **gliding re-entry** (WI 526) — it enters at an angle of attack, generates lift + transonic wave drag, and weathervanes to trim (HUD shows altitude/speed/medium, static hull pressure, and dynamic ram pressure / max-Q)
