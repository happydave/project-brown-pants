...
- `cargo run -p sounding -- workshop` — grounded build-and-test sandbox (WI 599/602/603/604/607/611/612/613/614): **Build** a craft with the **mouse** on a solid 0.1 m grid (left-click a face to add, right-click to remove, middle-drag orbit, scroll zoom; pick blocks/devices/parts from the on-screen **palette** down the left edge, or the keyboard: `Tab` material · `1`-`5` devices · `6`/`7` wheels · `8`/`9`/`0`/`-` seat/antenna/solar/bumper; each material renders with its own surface) ↔ **Test** — *what you built*: a build with **wheels drives as a rover** (solid chassis + steering tyres + parts; W/S drive · A/D steer · Space brake; **roll it** with a hard turn at speed or an obstacle clip, while normal driving stays upright; powered by an engine+tank → **fuel** or battery+solar → **charge**, shown in the HUD — run dry and it coasts; collides with **obstacles** on the pad), anything else **flies** on the textured ground with live collision (land, rest, crash → shatter); toggle with `Enter`, `Backspace` rebuilds
