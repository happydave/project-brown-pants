...
use crate::editor::{
    ...
    Brush, EditorState, HoverState, OrbitCam, PointerOnPalette,
};
...
impl Plugin for WorkshopScenePlugin {
    fn build(&self, app: &mut App) {
        app.add_plugins(FloatingOriginPlugin)
            ...
            .add_systems(OnExit(WorkshopMode::Build), build::exit_build)
...
fn draw_rover(mut gizmos: Gizmos, world: Res<WorkshopWorld>) {
    ...
    for (i, w) in rs.rover.wheels.iter().enumerate() {
        ...
        // Spin-out indicator (WI 650, recalibrated WI 651): tint the spokes cool→hot by |slip| so
        // wheelspin (which at high spin aliases into a static-looking blur) reads at a glance. Gripping
        // (slip ≲ 0.3, incl. the low-speed-inflated launch metric) stays grey; clear wheelspin glows red.
        let slip = ((w.slip_ratio.abs() - 0.3) / 0.7).clamp(0.0, 1.0) as f32;
