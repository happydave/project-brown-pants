...
impl Rover {
    ...
    pub fn step(&mut self, terrain: &Terrain, dt: f64) {
        ...
        // (`HULL_MAX_G`), kept low so the cliff-lip launch stays modest; the tunnel failure is handled by
        // the analytic backstop, not a higher cap. `n_hull` is the live contact count, so the per-contact
        // stiffness/cap divide to a build-size-independent total.
        ...
        let hull_contact = !hull_contacts.is_empty();
        let mut max_pen = 0.0_f64;
        for (p, normal, pen) in hull_contacts {
            ...
            // and at the impulse that would just arrest this contact's share of the slip this sub-step.
...
pub fn assemble_rover(craft: &VoxelCraft, position: DVec3, gravity: f64) -> Option<RoverAssembly> {
    ...
    // belly fallback for a degenerate (point-less) hull so the chassis never tunnels. Retained as the
    // fallback when no hull shape is available.
